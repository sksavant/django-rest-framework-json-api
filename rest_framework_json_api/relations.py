        ...
        if self.use_pk_only_optimization() and self.source_attrs:
            ...
            try:
                ...
                if is_simple_callable(value):
                    ...
                    value = value().pk
                if not self.queryset: # If remapping of related name
                    if self.field_name in self.parent.included_serializers.keys():
                        model = self.parent.included_serializers[self.field_name].Meta.model
                    print(self.field_name, model)
                else:
                    model = self.queryset.model
                return PKOnlyObjectWithMeta(pk=value, model=model)
